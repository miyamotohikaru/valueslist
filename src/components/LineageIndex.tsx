...
export default function LineageIndex({ lineages }: { lineages: Lineage[] }) {
  return (
    <nav aria-label="系譜の目次">
      <p className="text-[12px] font-bold text-vl-ink-soft">目次 · {lineages.length}本の系譜</p>
            ...
            <li key={l.id}>
              <a href={`#${l.id}`} className="vl-ticket group block h-full bg-vl-ink p-[2px] transition-transform duration-150 hover:-translate-y-0.5">
                <span className="vl-ticket grid h-full grid-cols-[minmax(0,1fr)_76px] bg-vl-card transition-colors group-hover:bg-vl-mustard">
                  ...
                  <span className="flex flex-col items-center justify-center border-l-2 border-dashed border-vl-ink py-2 pr-3 leading-none">
                    <span className="flex items-baseline">
                      {span.pre && <span className="text-[11px] font-bold">{span.pre}</span>}
                      <span className="font-display-en text-[22px] text-vl-red">{span.num || span.post}</span>
                      {span.num && <span className="text-[11px] font-bold">{span.post}</span>}
                    </span>
                  </span>
                </span>
              </a>
